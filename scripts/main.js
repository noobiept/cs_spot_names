...
function start()
{
var background = new createjs.Bitmap( G.PRELOAD.getResult( 'dust2_background' ) );

G.BACKGROUND_STAGE.addChild( background );

var map = 'dust2';
var mapSpots = MAPS[ map ];
var length = mapSpots.length;

for (var a = 0 ; a < length ; a++)
    {
    var spotInfo = mapSpots[ a ];
    var identifier = map + '_' + spotInfo.name;

    var spot = new createjs.Bitmap( G.PRELOAD.getResult( identifier ) );
    ...
    spot.on( 'rollover', function( event )
        {
        event.target.image = G.PRELOAD.getResult( event.target.identifier + '_selected' );
        G.MAIN_STAGE.update();
        });
    spot.on( 'rollout', function( event )
        {
        event.target.image = G.PRELOAD.getResult( event.target.identifier );
        G.MAIN_STAGE.update();
        });
